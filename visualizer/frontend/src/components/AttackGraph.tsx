...
const tokenBySeverity: Record<Severity, string> = { recon: "--sev-recon", "brute-force": "--sev-bruteforce", "auth-success": "--sev-authsuccess", "admin-access": "--sev-adminaccess", info: "--sev-info" };
...
export function AttackGraph({ graph, confidence, selectedId, onSelect, onHover }: { graph: GraphData; confidence: number; selectedId?: string; onSelect: (id?: string) => void; onHover: (id?: string) => void }) {
  ...
  return <div ref={host} className="graph-area" aria-label="Grafo tridimensional de eventos"><div className="graph-toolbar"><div className="legend" aria-label="Leyenda de severidad"><SeverityBadge severity="recon" /><SeverityBadge severity="brute-force" /><SeverityBadge severity="auth-success" /><SeverityBadge severity="admin-access" /></div><button className="button-secondary bg-[color-mix(in_srgb,var(--surface)_85%,transparent)]" onClick={reset}><RotateCcw size={15} />Restablecer vista</button></div>
    <ForceGraph3D ref={graphRef} width={size.width} height={size.height} graphData={data} backgroundColor={css("--bg")} showNavInfo={false} nodeThreeObject={makeNode} nodeLabel={() => ""} onNodeHover={(raw) => { const id = (raw as GraphNode | null)?.id; setHovered(id); onHover(id); }} onNodeClick={selectNode} onBackgroundClick={() => onSelect(undefined)} linkColor={(raw) => { const edge = raw as { source: GraphNode | string; target: GraphNode | string }; const source = typeof edge.source === "string" ? edge.source : edge.source.id; const target = typeof edge.target === "string" ? edge.target : edge.target.id; return activeId && (source === activeId || target === activeId) ? css("--text-tertiary") : css("--border-strong"); }} linkOpacity={activeId ? .55 : .6} linkWidth={(raw) => { const edge = raw as { source: GraphNode | string; target: GraphNode | string }; const source = typeof edge.source === "string" ? edge.source : edge.source.id; const target = typeof edge.target === "string" ? edge.target : edge.target.id; return activeId && (source === activeId || target === activeId) ? 2 : 1; }} enableNodeDrag={false} warmupTicks={80} cooldownTicks={120} />
  </div>;
}
