...
export function Analyses() {
  ...
  return <div className="page"><div className="page-heading"><div><h1 className="page-title font-display">Historial de análisis</h1><p className="page-subtitle">Reconstrucciones almacenadas para esta cuenta.</p></div><label className="relative block min-w-64"><Search size={16} className="absolute left-3 top-3 text-[var(--text-muted)]" /><input className="input pl-9" value={search} onChange={(event) => { setSearch(event.target.value); setOffset(0); }} placeholder="Buscar ID o fuente" aria-label="Buscar análisis" /></label></div>
