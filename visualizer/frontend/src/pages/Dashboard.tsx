...
import { analysisSeverity, eventSeverity, relativeTime } from "../lib/format";
...
export function Dashboard() {
  ...
  const feed = details.flatMap((query, index) => query.data?.events.slice(-1).map((event) => ({ event, analysisId: recent.data!.items[index]!.id })) ?? []).sort((a, b) => b.event.timestamp.localeCompare(a.event.timestamp));
  ...
  return <div className="page">
    ...
    <div className="dashboard-grid">
      <section className="card" aria-labelledby="activity-title"><div className="section-header"><div className="flex items-center gap-2"><Radio size={16} className="text-[var(--sev-recon)]" /><h2 id="activity-title" className="section-title">Actividad reciente</h2></div><span className="text-[var(--text-xs)] text-[var(--text-muted)]">Actualiza cada 10 s</span></div>
        {recent.data?.total === 0 ? <EmptyState title="Aún no hay actividad" detail="Configura un token de dispositivo para comenzar a recibir análisis." /> : <div className="feed">{feed.length ? feed.map(({ event, analysisId }) => <Link className="feed-row text-inherit no-underline" to={`/analyses/${analysisId}`} key={event.event_id}><SeverityBadge severity={eventSeverity(event)} /><div className="feed-main"><div className="feed-route"><span>{event.source_ip ?? "IP no registrada"}</span><span className="text-[var(--text-muted)]">→</span><span>{event.path ?? event.event_type}</span></div><div className="feed-meta">{event.raw_source} · {event.outcome ?? "evento observado"}</div></div><time className="timestamp" dateTime={event.timestamp}>{relativeTime(event.timestamp)}</time></Link>) : (recent.data?.items.map((item) => <Link className="feed-row text-inherit no-underline" to={`/analyses/${item.id}`} key={item.id}><SeverityBadge severity={analysisSeverity(item)} /><div className="feed-main"><div className="feed-route">{item.source_id} · {item.trace_id}</div><div className="feed-meta">{item.stages.join(" · ")}</div></div><time className="timestamp">{relativeTime(item.created_at)}</time></Link>))}</div>}
      </section>
      <section className="card" aria-labelledby="sources-title"><div className="section-header"><h2 id="sources-title" className="section-title">Fuentes</h2></div>{sources.length ? <div className="source-list">{sources.map(([source, count]) => <div className="source-row" key={source}><span className="font-mono text-sm text-[var(--text-secondary)]">{source}</span><span className="font-mono text-sm">{count}</span></div>)}</div> : <EmptyState title="Sin fuentes" detail="Las fuentes aparecerán después de la primera ingestión." />}</section>
    </div>
  </div>;
}
